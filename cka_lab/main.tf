...
resource "azurerm_network_interface" "cka-lab-nic-controller" {
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.cka-lab-pip-controller[count.index].id
  }
}

resource "azurerm_public_ip" "cka-lab-pip-controller" {
  count = var.cluster_size.controllers
...
resource "azurerm_availability_set" "aset_cka_lab" {
  name = var.aset_name
  location = var.location
  resource_group_name = azurerm_resource_group.cka-lab.name
...
resource "azurerm_virtual_machine" "controller" {
  ...
  availability_set_id = azurerm_availability_set.aset_cka_lab.id
  network_interface_ids = [azurerm_network_interface.cka-lab-nic-controller[count.index].id]
  ...
  storage_os_disk {
    name = "${var.prefix}-controller-${count.index}-osdisk"
  ...
  provisioner "file" {
    ...
    connection {
      ...
      host = azurerm_public_ip.cka-lab-pip-controller[count.index].fqdn
    }
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/controller/Documentation/kube-flannel.yml"
    ]
...
resource "azurerm_virtual_machine" "worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x /home/${var.admin_username}/setup.sh",
      "sudo /home/${var.admin_username}/setup.sh",
      "sudo kubeadm join ${azurerm_network_interface.cka-lab-nic-controller[0].ip_configuration[0].private_ip_address}:6443 --token ${var.kubeadm_token} --discovery-token-unsafe-skip-ca-verification"
    ]
